...
import { Calendar, ChevronRight, Clock, Heart } from 'lucide-react';

// ============================================================================
// LOCAL GIF IMPORTS
// These are bundled with the app so they work even when deployed.
// ============================================================================
...
// ============================================================================
// IMAGE MAP
// Keeps all GIF references in one place.
// ============================================================================
const IMAGES = {
...
// ============================================================================
// FOOD MESSAGES
// Random cute message shown when a food is selected.
// ============================================================================
const FOOD_MESSAGES = [
  'nice choice babe 😋',
  'u got it princess 👑',
...
// ============================================================================
// FOOD OPTIONS
// Shown on the food selection screen.
// ============================================================================
const FOOD_OPTIONS = [
...
// FORMSPREE CONFIG
// IMPORTANT:
// Each person should put THEIR OWN Formspree link in .env.local, not here.
//
// Example .env.local:
// VITE_FORMSPREE_ENDPOINT=https://formspree.io/f/your_own_form_id
//
// This makes sure each person receives only their own submissions.
// If everyone uses the same endpoint, everyone sends to the same inbox.
// ============================================================================
...
export default function App() {
  ...
  // Tracks how many times "No" was clicked
  const [noCount, setNoCount] = useState(0);

  // Controls when the letter appears on step 0
  ...
  // ==========================================================================
  // SUBMISSION STATE
  // Used for loading and error messages while sending to Formspree.
  // ==========================================================================
  ...
  // ==========================================================================
  // STEP 0 EFFECT
  // After 3.5s, show the letter after the walking cat animation.
  // ==========================================================================
  useEffect(() => {
    if (step !== 0) return;
  ...
  // ==========================================================================
  // HANDLER: NO BUTTON
  // Makes the "Yes" button grow and changes the cat image.
  // ==========================================================================
  const handleNoClick = () => {
    setNoCount((prev) => (prev < 3 ? prev + 1 : prev));
  };

  // ==========================================================================
  // HANDLER: YES BUTTON
  // Moves user to the happy screen.
  // ==========================================================================
  ...
  // ==========================================================================
  // HANDLER: FOOD SELECT
  // Saves chosen food and shows a random cute message.
  // ==========================================================================
  const handleFoodSelect = (foodId: string) => {
    setFoodChoice(foodId);
    const randomMsg =
      FOOD_MESSAGES[Math.floor(Math.random() * FOOD_MESSAGES.length)];
    setFoodMessage(randomMsg);
  };

  // ==========================================================================
  // HANDLER: DATE INPUT
  // Allows only digits and limits length for day/month/year.
  // ==========================================================================
  const handleDateChange = (field: keyof DateState, value: string) => {
  ...
  // ==========================================================================
  // HELPER: CAT IMAGE ON PROPOSAL SCREEN
  // Changes depending on how many times "No" was clicked.
  // ==========================================================================
  ...
  // ==========================================================================
  // HELPER: YES BUTTON SIZE
  // Gets bigger each time "No" is clicked.
  // ==========================================================================
  const getYesButtonSize = () => {
    return 1 + noCount * 0.5;
  };

  // ==========================================================================
  // HELPER: SELECTED FOOD NAME
  // Converts food id to readable name for email/final screen.
  // ==========================================================================
  const selectedFoodName =
    FOOD_OPTIONS.find((food) => food.id === foodChoice)?.name || foodChoice;

  // ==========================================================================
  // SEND DATA TO FORMSPREE
  //
  // VERY IMPORTANT:
  // This uses FORM_ENDPOINT from .env.local:
  // VITE_FORMSPREE_ENDPOINT=https://formspree.io/f/your_own_form_id
  //
  // Each person must set their own endpoint in their own .env.local file.
  // That way each person receives only their own form submissions.
  // ==========================================================================
  const sendEmailData = async () => {
    // Safety check in case someone forgot to add their Formspree link
    if (!FORM_ENDPOINT) {
      throw new Error(
        'Missing Formspree endpoint. Add VITE_FORMSPREE_ENDPOINT to your .env.local file.'
      );
    }
...
Time: ${time}
Food: ${selectedFoodName}`,
      _subject: 'New date app submission 💌',
    };
    ...
    if (!response.ok) {
      const errorMessage =
        ...
        data?.error ||
        'Failed to send submission to Formspree.';
  ...
  // Validates fields, sends to Formspree, then goes to final success screen.
  // ==========================================================================
  const handleFinalSubmit = async () => {
    setSubmitError('');

    if (!date.day || !date.month || !date.year || !time || !foodChoice) {
      setSubmitError('Please complete the date, time, and food choice first.');
      return;
    }
    ...
    try {
      ...
      setSubmitError(
        error instanceof Error
          ? error.message
          : 'Something went wrong while sending your form.'
      );
    } finally {
  ...
  return (
    <div className="min-h-screen bg-pink-50 flex flex-col items-center justify-center overflow-hidden font-sans text-gray-800">
      <AnimatePresence mode="wait">
              ...
              {showLetter && (
                ...
                >
                  <div className="flex flex-col items-center gap-4">
                    ...
                    />
                    <span className="text-pink-600 font-bold animate-pulse text-lg">
                      Click me!
                    </span>
                  </div>
                </motion.div>
                ...
                <motion.button
                  onClick={() => {
                    if (date.day && date.month && date.year && time) {
                      ...
                      alert(
                        'Please pick a date and time so I know when to get you! 😉'
                      );
                    }
                  }}
        ...
        {/* ====================================================================
            STEP 4: FOOD SCREEN + FINAL SUBMIT
            This is where the app sends data to Formspree when the button is clicked.
        ==================================================================== */}
        {step === 4 && (
          <motion.div
            key="step4"
          ...
          >
            ...
            <p className="text-pink-500 font-medium mb-4 text-center px-4 bg-pink-50 py-2 rounded-lg">
            ...
            {foodMessage && (
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                className="text-lg font-bold text-pink-600 mb-6 text-center bg-pink-100 py-3 px-6 rounded-full"
              >
                {foodMessage}
              </motion.div>
            )}
            ...
            {submitError && (
              <div className="mb-6 text-center text-sm text-red-500 bg-red-50 border border-red-200 px-4 py-3 rounded-xl mx-4">
                {submitError}
              </div>
            )}
            ...
            <AnimatePresence>
              {foodChoice && (
                <motion.button
                  ...
                  className="flex items-center gap-2 bg-pink-500 hover:bg-pink-600 disabled:bg-pink-300 text-white font-bold py-4 px-10 rounded-full shadow-xl transition-all mb-8"
                  whileHover={{ scale: isSubmitting ? 1 : 1.05 }}
                  whileTap={{ scale: isSubmitting ? 1 : 0.95 }}
                >
                  {isSubmitting ? 'Sending...' : "Perfect, let's go!"}
          ...
          >
            ...
            <div className="bg-white p-6 rounded-3xl shadow-xl w-full border-2 border-pink-100 relative overflow-hidden">
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-4 z-10 relative">
                I&apos;ll come get u at{' '}
                <span className="text-pink-500">{time}</span> <br />
                on{' '}
